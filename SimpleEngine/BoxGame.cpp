...
LRESULT BoxGame::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	return Game::BaseMsgProc(hwnd, msg, wParam, lParam);
}

void BoxGame::Update(const GameTimer& gt)
{
...
void BoxGame::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void BoxGame::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void BoxGame::BuildDescriptorHeaps()
{
...
void BoxGame::BuildShadersAndInputLayout()
{    
...
void BoxGame::BuildBoxGeometry()
{
 std::array<VPosData, 8> vPoses =
    {
	...
	std::array<VColorData, 8> vColors =
	{
    ...
    const UINT vbpByteSize = (UINT)vPoses.size() * sizeof(VPosData);
	const UINT vbcByteSize = (UINT)vColors.size() * sizeof(VColorData);
...
void BoxGame::Draw(const GameTimer& gt)
{
